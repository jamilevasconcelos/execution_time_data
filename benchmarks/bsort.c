...
// function to save data in saida_bsort_CI.csv
void save_data(uint64_t val_cycles, uint64_t val_instruction){

	FILE *arq;

	arq = fopen("saida_bsort.csv", "a");
...
int main( int argc, char ** argv){
  ...
  bsort_BubbleSort(arr, arr_size);

  //end mensure
  ioctl(fd_instruction, PERF_EVENT_IOC_DISABLE, PERF_IOC_FLAG_GROUP);
...
//  save_data(val_cycles, val_instruction, val_ev1, val_ev2, val_ev3, val_ev4, val_ev5);
