...
// function to save data in saida_msort.csv
void save_data(uint64_t val_cycles, uint64_t val_instruction){

	FILE *arq;

	arq = fopen("saida_msort.csv", "a");
...
void merge(int arr[], int l, int m, int r)
{
	...
	/* cria os arrays temporario left e right */
	int L[n1], R[n2];

	/* preenche os arrays temporarios */
	for (i = 0; i < n1; i++){
		L[i] = arr[l + i];
	}
	for (j = 0; j < n2; j++){
	...
	/* finaliza a troca a depender do i e j incrementados anteriormente */
	while (i < n1){
	   arr[k] = L[i];
	   i++;
	   k++;
...
/* l eh o indice inicial e r o final*/
void mergeSort(int arr[], int l, int r)
{
	if (l < r){
		...
		// divide a primeira e a segunda parte do vetor
		mergeSort(arr, l, m);
		mergeSort(arr, m+1, r);
...
int main( int argc, char ** argv){
  ...
  // //INICIO GERA VETOR#####
  //  srand(time(0));
  //  int iter_max = 1000;// + (rand() % 1500);
  //  int b;
  //  int arr[iter_max];


  //  for(b = 0; b<iter_max; b++){
	// 	int numero = rand() % 15000;
	// 	arr[b] = numero;
  //  }
